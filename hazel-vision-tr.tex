...
problem, and some aspects of this problem remain open. There is reason to be optimistic, however, with recent studies 
suggesting that programmers experienced with a keyboard-driven structure editors (e.g. \texttt{mbeddr}) 
can be highly productive \cite{DBLP:conf/vl/Asenov014,DBLP:conf/sle/VolterSBK14}.

Researchers have also explored ``hybrid'' approaches. For example, recent work on \emph{syntactic placeholders} envisions 
an editor where many edit actions cause textual placeholders to appear, rather than leaving the program transiently malformed \cite{Amorim:2016:PSC:2997364.2997374}. This 
``approximates'' the experience of a structure editor in common usage, while allowing the programmer to perform arbitrary 
text edits if desired. A related approach would be to perform error recovery by attempting to insert placeholders into the internal representation used 
by the program editor, without including them in the surface syntax used by programmers. If ``pure'' structure editing proves too rigid as we design \Hazel, we will explore hybrid approaches like these.
