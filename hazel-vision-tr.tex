...
\titlerunning{Toward Semantic Foundations for Interactive Programming Tools} %optional, in case that the title is too long; the running title should fit into the top page column
...
with \emph{incomplete} programs -- programs with holes, type inconsistencies, binding inconsistencies and other transient problems -- using tools like program editors and
live programming environments (which interleave editing and
evaluation.)
%
Formalists have done little to formally characterize 1) the static and dynamic semantics of incomplete programs; 2) the 
actions that programmers perform as they edit and inspect incomplete programs; and 3) the behavior of editor services designed to help the programmer discover and combine these edit actions. % Each tool designer has largely been left to develop their own \emph{ad hoc} heuristics. 
%

This paper serves as a vision statement for the HazelGrove project, which seeks to develop these ``missing'' semantic 
foundations. We hope that these contributions, which will take the form of a series of typed calculi, will guide the design of a variety of future interactive programming tools, much as various typed lambda calculi have guided modern language designs. In our own research, we plan to apply these principles in the design of \HazelEnv, a clean-slate \emph{live lab notebook} structure editor. 
