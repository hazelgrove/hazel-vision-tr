...
evaluation.)
%
Semanticists have done little to formally characterize 1) incomplete programs; 2) the 
actions that programmers perform on incomplete programs; and 3) the behavior of editor services designed to assist the programmer as they perform these actions. Each tool designer has largely been left to develop their own \emph{ad hoc} heuristics. 
%

This paper serves as a vision statement for the HazelGrove project, which seeks to develop these ``missing'' semantic 
foundations. We hope that these contributions, which will take the form of a series of typed calculi, will guide the design of a variety of future interactive programming tools, much as various typed lambda calculi have guided a variety of modern language designs. In our own research, we plan to apply these principles in the design of \HazelEnv, a clean-slate \emph{live lab notebook} structure editor. 
