...
and therefore programs, as mathematical structures~\citep{pfpl,Pierce:2002hj}; and (2) the design
...
extensions installed \citep{gamma2004contributing}), will provide a number of standard editor services, including (1)
...
language \citep{DBLP:conf/tools/KrahnRV08,DBLP:conf/cc/BrandDHJJKKMOSVVV01}. The remaining editor services could be implemented
...
complete program must be disabled. This is unsatisfying. Sophisticated editors like Eclipse, and editor generators like Spoofax \citep{DBLP:conf/oopsla/KatsV10}, use \emph{error recovery} heuristics that can support the continued provision 
of various services in some formally malformed states, by using secondary notational conventions (e.g. whitespace) to segregate malformed portions of the program  \citep{DBLP:conf/oopsla/KatsJNV09,DBLP:conf/sle/JongeNKV09}. These heuristics work most, but not all, of the time.
...
evaluation \citep{DBLP:conf/icse/Tanimoto13,DBLP:journals/vlc/Tanimoto90,McDirmid:2007:LUL:1297105.1297073}. These tools provide programmers with more rapid feedback about the
dynamic behavior of the program they are editing, or selected portions thereof \citep{McDirmid:2013:ULP:2509578.2509585}. Examples include \emph{lab notebooks},
e.g. the popular IPython/Jupyter~\citep{Perez:2007:ISI:1251563.1251831}, which allow the
programmer to interactively edit and evaluate program fragments organized into a
sequence of cells (an extension of the read-eval-print loop (REPL)); spreadsheets; {live graphics programming environments}, e.g. SuperGlue \citep{McDirmid:2007:LUL:1297105.1297073}, Sketch-n-Sketch \citep{DBLP:conf/pldi/ChughHSA16,DBLP:conf/icse/Chugh25} and the tools demonstrated by Bret Victor in his lectures \citep{victor2012inventing}; the TouchDevelop live UI framework \citep{burckhardt2013s}; and live visual and auditory dataflow languages \citep{DBLP:conf/vl/BurnettAW98}. 
...
from theoretical work on gradual typing~\citep{Siek06a}. We seek a dynamic semantics for the
...
and binding errors would violate the usual notions of type safety. As such we plan to develop a \emph{mechanized
  metatheory}~\citep{Lee:2007:TMM:1190216.1190245}: we will use a proof assistant to mechanize the semantics and
...
the sense of Milner to mean that it will not have undefined behavior~\citep{milner1978theory,pfpl}).
This represents the ``gold standard'' for evaluating claims in formal
semantics.  Our preliminary work has used Agda~\citep{norell2009dependently}.
...
edit actions will be bound to keyboard shortcuts (studies suggest that programmers experienced with a keyboard-driven structure editors can be highly productive \citep{DBLP:conf/vl/Asenov014}). However, \HazelEnv will 
...
using the Agda theorem prover~\citep{norell2009dependently}.
...
enable/disable the \HazelEnv-specific features \citep{wizard-of-oz}. We will also use 
a think-aloud protocol~\citep{think-aloud-protocol}, in which participants are 
...
experiments~\citep{stylos-constructor-params, static-typing-apidoc, benefits-generic-types}.
