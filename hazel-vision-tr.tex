...
Figure \ref{fig:hazel-mockup} shows a mockup of the \Hazel ~user interface, which is loosely modeled on the widely adopted IPython / Jupyter lab notebook interface~\cite{PER-GRA:2007}. 
...
suggesting that programmers experienced with a keyboard-driven structure editors 
...
an editor where many edit actions cause textual placeholders to appear, rather than leaving the program malformed \cite{Amorim:2016:PSC:2997364.2997374}. This 
``approximates'' the experience of a structure editor in common usage, while allowing the programmer to perform arbitrary 
text edits if desired. If a pure structure editor based approach proves too rigid for Hazel, we will explore such hybrid approaches. 
