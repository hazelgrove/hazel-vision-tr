...
\[\texttt{matrix(float) -> \{ {mean} : vec(float), std : vec(float), median :~$\square$ \}}\] %Notice that holes can appear in both expressions and types.
...
Research on CMTT has not yet 
 ...
 %Accomplishing this goal will require great formal care, because the presence of type
% and binding errors would violate the usual notions of type safety -- evaluation can ``get stuck'' because a hole appears in a critical (elimination) position. 
